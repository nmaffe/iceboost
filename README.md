...
From [Global Terrestrial Network for Glaciers](https://www.gtn-g.ch/data_catalogue_glathida/) download
the ```glathida-3.1.0.zip``` into a folder called ```glathida/```. Unzip the archive.
...
```
In ```create_metadata.py``` you can specify as arguments all input products that you have downloaded.
The code generates a raw ~3.0 GB ```.csv``` training dataset.
...
```
This code removes some points from the raw training dataset, and performs a per-glacier spatial downscaling
of all features into a 100x100 grid mesh. The code produces the final ~300.0 MB ```.csv``` file for training.
